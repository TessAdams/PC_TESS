...
println(val);
  ...
  //background(165, 136, 105,.01);
   ...
   //stroke(234,192,134,100); //cool effect
     fill(255,15);
     directionalLight(255, 205, 148, 8, 10, -5);
    ...
    //if(keyPressed){
       ...
       stroke(234,192,134,100); //cool effect
      pointLight(20, 20, 100, 100, 100, 0); //color change #1, Heaven
       filter (ERODE); //slows down framerate 
       //filter (DILATE);
       //filter (POSTERIZE,40);
       
       
       ambientLight(100,30,10);
     ...
     rotateX(PI/2.8);//rotate plane
  translate(-w/2,-h/1.6);
  for(int y = 0; y < rows-1; y++) {
    beginShape(TRIANGLE_STRIP);
    for(int x = 0; x < cols; x++) { 
      vertex(x*scale,y*scale, terrain[x][y]); //put the z value at x, y
      vertex(x*scale,(y+8)*scale, terrain[x][y]); //put z value at x, y+1
    }
    endShape();
  }
}